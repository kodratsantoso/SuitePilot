...
import { useTechnicalWorkstreams, useCreateTechnicalWorkstream, useUpdateTechnicalWorkstream } from '@/hooks/useTechnicalWorkstreams'
...
function WorkstreamForm({
  ...
  initial?: Partial<TechnicalWorkstream>
    ...
    >
      <div>
        ...
        <input
          ...
          required
          placeholder="e.g. Integration Workstream"
        />
      </div>
      <div>
        ...
        <textarea
          ...
          placeholder="Workstream scope and technical objectives..."
        />
      </div>
      <div>
...
export default function TechnicalWorkstreamsPage() {
  const { projectId } = useParams<{ projectId: string }>()
  const { data: workstreams = [], isLoading, isError, refetch } = useTechnicalWorkstreams(projectId)
  const createWorkstream = useCreateTechnicalWorkstream(projectId)
  const updateWorkstream = useUpdateTechnicalWorkstream(projectId)

  const [sheetOpen, setSheetOpen] = useState(false)
  const [editing, setEditing] = useState<TechnicalWorkstream | undefined>()
  ...
  if (isLoading) return <LoadingState message="Loading technical workstreams..." />
  if (isError) return (
    <div className="text-red-600 text-sm">
      Failed to load workstreams. <button onClick={() => refetch()} className="underline">Retry</button>
    </div>
  )
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Technical Workstreams"
        description="Manage technical implementation workstreams and track delivery progress"
        action={<Button size="sm" onClick={() => { setEditing(undefined); setSheetOpen(true) }}>+ Add Workstream</Button>}
      />

      {workstreams.length === 0 ? (
        <EmptyState
          title="No technical workstreams yet"
          description="Create workstreams to organise your technical delivery by integration area."
          ...
          {workstreams.map((ws) => (
            <div key={ws.id} className="rounded-lg border border-gray-200 bg-white p-5">
              ...
              <div className="flex gap-3 text-xs text-gray-400">
                <span>{ws._count?.technicalDeliverables ?? 0} deliverables</span>
              </div>
              <button
      ...
      <Sheet open={sheetOpen} onClose={() => { setSheetOpen(false); setEditing(undefined) }} title={editing ? 'Edit Workstream' : 'New Technical Workstream'}>
        <WorkstreamForm
          initial={editing}
